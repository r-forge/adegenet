...
\color{black}
\noindent
{\small Please ask questions here: \url{adegenet-forum@lists.r-forge.r-project.org}.}\\
\noindent
{\small Please make comments there: \url{t.jombart@imperial.ac.uk}.}
\tableofcontents
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Current version of the package is 1.2-3, and is compatible with R 2.8.1.
Please make sure to be using at least R 2.8.1 and adegenet 1.2-3
before sending question about missing functions.
...
from a \rcmd{data.frame} of genotypes, by conversion from a table of
...
Alternatively, one can use the accessor \rcmd{locNames}:
<<>>=
locNames(nancycats)
...
\noindent Note that the slot 'pop' can be retrieved and set using the \rcmd{pop} function:
<<>>=
...
Finally, a \rcmd{genind} object generally contains its matched call, \textit{i.e.} the instruction that created it.
...
markers table where each element is a character string coding the
alleles possessed at one locus.
Such data are interpretable when all strings contain even numbers of
characters, provided the right degree of ploidy is specified.
For instance, in a diploid organism, "11" will be an homozygote 1/1, "1209" will be an heterozygote 12/09.
In a tetraploid system, "1209" will be understood as 1/2/0/9.
Of course, data coded with separators between alleles are much more
explicit, and are handled too.
The function \rcmd{df2genind} converts all these kinds of data to a \rcmd{genind}.
...
class \rcmd{genotype} is still used in various packages.
...
diploid organisms, \rcmd{hybridize} is not retrained to this case.
...
dat
@
\noindent The function \rcmd{df2genind} is used to obtain a genind object:
<<>>=
...
the Monmonier's algorithm (\rcmd{monmonier}) will work just fine
...
Yet, when trying to use some functions, like \rcmd{read.genetix} for
...
version of adegenet.
This can be because you did not update your packages (see function \rcmd{update.packages}).
